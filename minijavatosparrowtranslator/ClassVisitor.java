...
public class ClassVisitor extends GJVoidDepthFirst<ClassSymbol>{
    ...
    */
    ...
    public SparrowFile getSubObject(String childclass, String target, String thisObjectVariable){
        ClassSymbol current = getClassFromName(childclass);
        int count = 0;
        ...
        ArrayList<String> lineList = newFile.lines;
        lineList.add(currentVariable + SparrowFile.equals() + thisObjectVariable);
        for(int i =0; i< count; i++){
            lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(4))));
        }

        return newFile;
    }

    public SparrowFile getMemberVariable(String className, String targetID, String thisObjectVariable, boolean leftSide){
        SparrowFile memberVarCodeGen = new SparrowFile();
        ArrayList<String> lineList = memberVarCodeGen.lines;
        ClassSymbol current = getClassFromName(className);
        int count = 0;
        int offset = 12;
        boolean found = false;

        while(current != null){            
            for(TypeSymbol memberVars : current.memberVariables){
                if(memberVars.identifier.equals(targetID)){
                    found = true;
                    break;
                }
                offset += 4;
        ...
        //zap to correct class pointer 
        String currentVariable = SparrowFile.randomVarName(); 
        lineList.add(currentVariable + SparrowFile.equals() + thisObjectVariable);
        for(int i =0; i< count; i++){
            lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(4))));
        }
        //zap to correct offset, grab variable, place it in some new variable
        if(leftSide){
            memberVarCodeGen.var = SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(offset)));
        }
        else{
            lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(offset))));
            memberVarCodeGen.var = currentVariable;
        }

        return memberVarCodeGen;

    }

    public SparrowFile getMemberMethod(String className, String targetID, String thisObjectVariable){
        SparrowFile memberVarCodeGen = new SparrowFile();
        ArrayList<String> lineList = memberVarCodeGen.lines;
        ClassSymbol current = getClassFromName(className);
        int count = 0;
        ...
        while(current != null){            
            for(MethodSymbol memberVars : current.methodSet){
                if(memberVars.name.equals(targetID)){
                    found = true;
                    memberVarCodeGen.retType = memberVars.returnType;
                    break;
                }
                offset += 4;
            }
        ...
        //zap to correct class pointer 
        String currentVariable = SparrowFile.randomVarName(); 
        String classPointerVariable = SparrowFile.randomVarName();
        lineList.add(currentVariable + SparrowFile.equals() + thisObjectVariable);
        for(int i =0; i< count; i++){
            lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(4))));
        }
        lineList.add(SparrowFile.equalsStmt(classPointerVariable, currentVariable));
        memberVarCodeGen.classPtr = classPointerVariable;
        //zap to correct offset, grab variable, place it in some new variable
        lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(8))));
        lineList.add(currentVariable + SparrowFile.equals() + SparrowFile.brackets(SparrowFile.addition(currentVariable, Integer.toString(offset))));
        memberVarCodeGen.var = currentVariable;
    ...
    /**
        ...
        */
        ...
        public void visit(FormalParameter n, ClassSymbol my) {
            ...
            additive.sparrowVariable = SparrowFile.randomVarName();
